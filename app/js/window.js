...
var Window = (function () {
    ...
    self.appendWelcome = function () {
        ...
        for (var k = 0; k < messages["title"].length; k++) {
            message2.appendChild(self.createClass("span", "span-color" + (k + 2), "color", k + 2, messages["title"][k].replace("-wifi-", config['robot']['ssid'])));
        }
    ...
    self.updateMessage = function (messageId) {
        ...
        for (var i = 0; i < messages[messageId].length; i++) {
            message.appendChild(self.createClass("span", "span-color" + (i + 1), "color", i + 1, messages[messageId][i].replace("-wifi-", config['robot']['ssid'])));
        }
    ...
    self.createLoader = function (status) {
        ...
        for (var j = 0; j < messages[status].length; j++) {
            message.appendChild(self.createClass("span", "span-color" + (j + 1), "color", j + 1, messages[status][j].replace("-wifi-", config['robot']['ssid'])));
        }
        if (status === "success") {
